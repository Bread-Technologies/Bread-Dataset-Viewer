...
| R6: classical GPTs cannot support nontrivial StrictConnectedAgency | 1 | ▲ PARTIAL+ | n=2 disconnect ✓ VERIFIED hypothesis-free (`classical_n2_strict_reversible_path_id_swap_empty`); n=3 sign-of-det ✓ VERIFIED via `n3_disc_det`/`classical_n3_strict_path_id_swap01_det_zero` (modulo deferred `n3_det_zero_implies_not_injective_conjecture`); n=3 conditional disconnect via `classical_n3_no_strict_path_id_to_swap01_conditional`; general-n still ? OPEN. Plus PatternStableWantable axiom refinement (Ontology.lean) provides discriminating Stable_nontrivial predicate. |
