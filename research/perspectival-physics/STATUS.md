...
| `Decoherence.IsCoherent`, `IsPureDecoherent`, `IsMixed` predicates with Decidable instances; `coherence_predicate_certificate` (nil, loops, append-iff, equivalence-respecting, endpoint-iff); `trichotomy_predicates_certificate` (exhaustive + pairwise exclusive) | `Decoherence.lean` | ✓ (a) **named decidable trichotomy API** |
