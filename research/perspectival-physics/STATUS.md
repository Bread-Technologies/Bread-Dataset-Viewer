...
| R6: classical GPTs cannot support nontrivial StrictConnectedAgency | 1 | ✓✓ VERIFIED for n=2 AND n=3 | n=2: `classical_n2_strict_reversible_path_id_swap_empty` (hypothesis-free). n=3: `classical_n3_no_strict_path_id_to_swap01` (unconditional, this session, via Matrix.det bridge — `n3_det_zero_implies_not_injective` PROVEN). General n ≥ 4 still ? OPEN (analogous via same Matrix.det approach). Plus PatternStableWantable axiom refinement (Ontology.lean) provides discriminating Stable_nontrivial predicate. |
