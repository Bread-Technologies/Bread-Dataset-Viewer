...
structure Frame (ι : Type*) [Fintype ι] (V : Type*)
...
def Decomposes {ι : Type*} [Fintype ι] (F : Frame ι V) (ω : V) (p : ι → ℝ) :
    Prop :=
...
determined. -/
theorem weight_eq_distinguish {ι : Type*} [Fintype ι] (F : Frame ι V)
...
theorem decomposition_unique {ι : Type*} [Fintype ι] (F : Frame ι V)
...
def Compatible {ι κ : Type*} [Fintype ι] [Fintype κ]
...
def Incompatible {ι κ : Type*} [Fintype ι] [Fintype κ]
    (F : Frame ι V) (G : Frame κ V) : Prop :=
  ∃ i j, F.distinguish i (G.actual j) ≠ 0 ∧ F.distinguish i (G.actual j) ≠ 1

theorem not_compatible_of_incompatible {ι κ : Type*} [Fintype ι] [Fintype κ]
    {F : Frame ι V} {G : Frame κ V} (h : Incompatible F G) :
    ¬ Compatible F G := by
  obtain ⟨i, j, h0, h1⟩ := h
  intro hc
...
def PerspectiveRelative {ι κ : Type*} [Fintype ι] [Fintype κ]
...
theorem actuality_bridge :
    ...
    (∀ {ι : Type} [Fintype ι] (F : Frame ι V) {ω : V} {p q : ι → ℝ},
    ...
    (∃ (W : Type) (_ : AddCommGroup W) (_ : Module ℝ W)
        (F G : Frame (Fin 2) W) (ω : W), PerspectiveRelative F G ω) :=
  ⟨fun F hp hq => decomposition_unique F hp hq,
